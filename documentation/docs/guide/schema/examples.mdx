...
In the previous sections we showed you how you can [call()](call.mdx) or [post()](post.mdx) function requests. We also
...
Each test has to set up the chain/contract/context from scratch. We will often
use a specific setupTest() function to do all setup work that is shared by many tests.

We cannot use the `dividendMember` wrapper function in these two tests
...
After the final `divide` call, we ended up with the exact amounts to disperse,
so no remainder iotas were left in the contract account.

Each divide is cumulative to the balances of the members. We have
highlighted this by indicating the separate increases after every `divide` call.

Finally, we will show how to test [Views](views.mdx) and/or [Funcs](funcs.mdx) that return a result.
Since solo executes [post()](post.mdx) requests synchronously, it is possible to have a Func return a result and
...
In the [next section](timelock.mdx) we will describe a few more helper member functions of the
SoloContext.

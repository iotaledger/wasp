...
- tokens
- incoming
description: There are two methods in the ISCP function context that deal with token balances. The balances() method can be used to determine the current total balance per token color. The incoming() method can be used to determine the amounts of incoming tokens per token color that were sent with the request to call the smart contract function.
...
There is also a `transfer_to_address()` method in the ISCP function context that can

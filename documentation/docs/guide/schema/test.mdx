...
Testing of smart contracts happens in the [Solo testing environment](../solo/what-is-solo.md). This enables
synchronous, deterministic testing of smart contract functionalities without the overhead of
...
directly. Because they run in a sandboxed environment our Wasm smart contracts cannot access these types directly.
Therefore, WasmLib implements its [own versions](../wasm_vm/types.mdx) of these data types, and the VM layer acts as a data type
...
programming for Rust and Go are practically identical, we feel that this is not unsurmountable.
They only differ where language idiosyncrasies force differences in syntax or naming
conventions. This hurdle used to be a lot bigger, when direct programming of Solo
had to be used, and type conversions had to be done manually. Now we get to use
...
In the [next section](examples.mdx) we will go deeper into how the helper member functions of the

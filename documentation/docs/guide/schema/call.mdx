...
In addition, when the function that is called is not a [View](views.mdx), it is possible to pass tokens
...
Otherwise, the incoming() function will not register any incoming tokens.

When the call is made, the calling function will be paused and wait for the called function
...
When calling a function from a View function, it is only possible to call other View functions. The

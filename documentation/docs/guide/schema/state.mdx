...
The `state` section in the schema file contains a number of field definitions that together
...
an AgentID. Both are predefined [WasmLib value types](types.mdx).
...
section in the schema file. Each member function accesses a type-safe proxy object for the

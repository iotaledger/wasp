...
The [schema tool](usage.mdx) allows you to define your own structured data types that are composed of
the predefined WasmLib value data types. The tool will generate a struct with named fields
according to the definition in the schema definition file, and also generate code to
serialize and deserialize the structure to a byte array, so that it can be saved as a
single unit of data, for example in state storage.

You can use structs directly as a type in state storage definitions and the schema
...
, the number of the item that was bet on, and the agent ID of the one who placed
...
In the next section we will look at how to make even more [complex type definitions](typedefs.mdx).

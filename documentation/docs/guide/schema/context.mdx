...
When the host calls a smart contract function it has to do it by invoking a second
...
This example also showcases an important feature of the contexts: the `log()` method.
...
In the next section we will introduce the [`schema`](schema.mdx) tool that simplifies smart contract
programming a lot.

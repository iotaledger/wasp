...
allows you to specify a single container type.
...
way you can easily create containers that contain such container types. The [Schema Tool](usage.mdx)
...
The schema tool will generate the following code in `typedefs.xx` for the BettingRound
type:
...
definition that is generated in `state.xx`:

...
contracts and simulate transactions.

Solo directly interacts with the ISC code, and uses all the data types that are
[defined in the ISC code](https://github.com/iotaledger/wasp/blob/develop/documentation/docs/misc/coretypes.md)
directly. Because they run in a sandboxed environment our Wasm smart contracts cannot
access these types directly. Therefore, WasmLib implements its [own versions](types.mdx)
of these data types, and the VM layer acts as a data type translator between both systems.
...
keeping the full Solo interface hidden as much as possible, but available when necessary.
...
As you can see, we pass the SoloContext and the parameters to the wrapper functions, then
...
There is hardly any difference in the way the function interface is used with the ISC

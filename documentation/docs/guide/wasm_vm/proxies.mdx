...
As we cannot call the ISCP sandbox functions directly, we need a library to access the
sandbox functionality. There is no way for the Wasm code to access any memory outside
its own memory space. Therefore, any data that is governed by the ISCP sandbox has to be
...
actual objects or values stored on the ISCP host, and know how to manipulate them. Proxies
...
underlying ISCP sandbox provides access to its data in the form of simple key/value stores

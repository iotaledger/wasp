...
In the previous sections we showed how you can [call()](call.mdx) or [post()](post.mdx)
...
on its Tangle address and its on-chain account.
...
the Params initialization we wanted to be missing.
...
Then in the next lines we update the asset balances with the changes we expect to
happen because of the call to the `member` function. And then we verify these changes
against the actual asset balances by calling `bal.VerifyBalances(t)`.
...
be used normally. Since solo executes `post()` requests synchronously, it is possible to

...
that is by providing a small, self-contained library that can be linked to the Wasm
code. This library is called `WasmLib`.
...
access seamless to the smart contract by hiding the details of bridging the gap
between the smart contract's memory space, and the otherwise inaccessible memory space
of the ISC host.

...
- store
- state
description: The Iota Smart Contracts Protocol (ISCP) provides  a very flexible way of programming smart contracts by providing an API to a sandboxed environment that allows you to interact with the ISCP deterministically without any security risks.
image: /img/schema_tool/IscpHost.png
---
# Introduction to the Wasm VM for ISCP
...
[![Wasp node ISCP Host](/img/schema_tool/IscpHost.png)](/img/schema_tool/IscpHost.png)
...
only provide access to the ISCP host's sandbox API environment. The way we do that is by
...
[![Wasm VM](/img/schema_tool/WasmVM.png)](/img/schema_tool/WasmVM.png)
...
fully functional [TinyGo](https://tinygo.org/) version.

Both implementations use only a very small common subset of their host languages. This
...
should quickly feel comfortable writing smart contracts in either language, without having
to dive deeply into all aspects of the chosen language.

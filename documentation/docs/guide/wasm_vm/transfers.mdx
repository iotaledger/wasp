...
description: There are two methods in the ISC function context that deal with token balances. The balances() method can be used to determine the current total balance per token color. The incoming() method can be used to determine the amounts of incoming tokens per token color that were sent with the request to call the smart contract function.
...
one is the `balances()` method, which can be used to determine the current asset balances
that are governed by the smart contract. The second one is the `allowance()` method, which
...
There is also a `transfer_allowed()` method in the ISC function context that can transfer
assets from the caller's on-chain account to any other on-chain account. The assets to be
transferred are provided to the method through a special `ScTransfer` proxy, which is
essentially a mutable version of `ScBalances`. We will be using the `transfer_allowed()`
method in the `dividend` example to disperse the incoming iotas to the member accounts.

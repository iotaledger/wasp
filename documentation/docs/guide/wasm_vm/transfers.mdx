...
There is also a `transfer_allowed()` method in the call context that can transfer assets
from the caller's on-chain account to any other on-chain account. The assets to be

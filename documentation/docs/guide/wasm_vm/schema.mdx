...
contract. This schema definition file is then used by our `Schema tool` to automatically
generate a complete smart contract skeleton that reflects the schema definition and only
needs to be augmented by providing the actual function implementations.
...
code so that there is a single, consistent way of calling smart contract functions.

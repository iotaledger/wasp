...
through a node API. We call such requests _off-ledger_ requests, as opposed to the
_on-ledger_ requests that are posted directly on the Tangle ledger. Off-ledger requests
can be sent at a much higher frequency than off-ledger requests, but on-ledger requests
offer a few additional features that are not available to off-ledger requests.

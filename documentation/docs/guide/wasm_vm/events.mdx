...
The schema tool will generate a structure that will become part of all Func function
contexts. Events can only be triggered from within a Func. They will become part of the
state of the smart contract because every event is logged in the core `blocklog` contract.
Therefore, events cannot be triggered from within a View.
...
In the next section we will explore how the schema tool helps to simplify smart contract

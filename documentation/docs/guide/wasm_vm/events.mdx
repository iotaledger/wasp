...
[function call context](context.mdx) exposes this interface through its `event()`
...
smart contract because every event is logged in the core `blocklog` contract. Therefore,
events cannot be triggered from within a View.

...
function call, but instead of using the `call()` method of the `func` member in the
function descriptor, you will now use the `post()` or `postToChain()` methods. The
...
same way we did previously with [syncronous calls](call.mdx), or you can have WasmLib
...
required, but if you post to a function that does not expect any tokens then you can
have WasmLib send the minimum amount for you.

**Providing a `delay()` before a `call()` will result in a panic**. We do not know the
...
should not really be a problem because using `delay()` is rare and using it with `call()`

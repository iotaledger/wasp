...
pub struct TokenBalanceContext {
    results: MutableTokenBalanceResults,
    state: ImmutableTestWasmLibState,
}

fn view_token_balance_thunk(ctx: &ScViewContext) {
    ctx.log("testwasmlib.viewTokenBalance");
    let f = TokenBalanceContext {
        results: MutableTokenBalanceResults { proxy: results_proxy() },
        state: ImmutableTestWasmLibState { proxy: state_proxy() },
    };
    view_token_balance(ctx, &f);
    ctx.results(&f.results.proxy.kv_store);
    ctx.log("testwasmlib.viewTokenBalance ok");
}

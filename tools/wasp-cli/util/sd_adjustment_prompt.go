...
func SDAdjustmentPrompt(output iotago.Output) {
	...
	if output.Deposit() < minStorageDeposit {
		...
		if (fi.Mode() & os.ModeCharDevice) == 0 {
			log.Fatal("transaction not sent: base tokens is less than the minimum storage deposit (auto-adjust with --adjust-storage-deposit)")
		}
...
`, minStorageDeposit, output.Deposit(), minStorageDeposit)
		...
		if strings.ToLower(text) != "y" {
			log.Fatal("transaction not sent.")
		}
	}
}

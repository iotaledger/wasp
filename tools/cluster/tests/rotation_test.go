...
func TestRotation(t *testing.T) {
	...
	chain, err := clu.DeployChain(clu.Config.AllNodes(), rotation1.Committee, rotation1.Quorum, rotation1.Address, false)
	require.NoError(t, err)
...
func TestRotationMany(t *testing.T) {
	...
	chain, err := clu.DeployChain(clu.Config.AllNodes(), rotations[0].Committee, rotations[0].Quorum, rotations[0].Address, false)
	require.NoError(t, err)
	t.Logf("chainID: %s", chain.ChainID)

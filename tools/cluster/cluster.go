...
func (clu *Cluster) DeployDefaultChain() (*Chain, error) {
	...
	return clu.DeployChainWithDistKeyGeneration(committee, committee, uint16(quorum))
}

func (clu *Cluster) InitDistKeyGeneration(committeeNodeCount int) ([]int, *cryptolib.Address, error) {
	cmt := util.MakeRange(0, committeeNodeCount-1) // End is inclusive for some reason.
	quorum := uint16((2*len(cmt))/3 + 1)

	address, err := clu.RunDistKeyGeneration(cmt, quorum)

	return cmt, address, err
}

func (clu *Cluster) RunDistKeyGeneration(committeeNodes []int, threshold uint16, timeout ...time.Duration) (*cryptolib.Address, error) {
	if threshold == 0 {
	...
	return apilib.RunDistKeyGeneration(context.Background(), client, peerPubKeys, threshold, timeout...)
}

func (clu *Cluster) DeployChainWithDistKeyGeneration(allPeers, committeeNodes []int, quorum uint16, blockKeepAmount ...int32) (*Chain, error) {
	stateAddr, err := clu.RunDistKeyGeneration(committeeNodes, quorum)
	if err != nil {
		return nil, err
	}

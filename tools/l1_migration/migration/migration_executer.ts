...
export interface MigratedAssets {
  NativeToken: Record<string, boolean>;
  NFTs: Record<string, boolean>;
  Foundries: Record<string, boolean>;
}

export async function createMigrationTransaction(client: IotaClient, iscPackageId: string, governorAddress: string, aliasId: string, anchorId: string): Promise<[tx: Transaction, migratedObjects: MigratedAssets ]> {
  ...
  let migratedObjects: MigratedAssets = {
    NativeToken: {},
    NFTs: {},
    Foundries: {}
  }
  ...
  for (let basic of assets.basics) {
    ...
    for (const nativeToken of nativeTokens!) {
      ...
      migratedObjects.NativeToken[typeArguments] = true;
    }
  ...
  for (let nft of assets.nfts) {
    ...
    migratedObjects.NFTs[nft.objectId] = true;
  ...
  for (let foundry of assets.foundries) {
    ...
    migratedObjects.Foundries[foundry.type!] = true;
  }

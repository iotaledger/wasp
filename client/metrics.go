...
func (c *WaspClient) GetChainNodeConnectionMetrics(chID *iscp.ChainID) (*model.NodeConnectionMessagesMetrics, error) {
	...
	if err := c.do(http.MethodGet, routes.GetChainNodeConnectionMetrics(chID.AsAddress().Bech32(iscp.NetworkPrefix)), nil, ncmm); err != nil {
		return nil, err
	}
	return ncmm, nil
}
...
func (c *WaspClient) GetChainConsensusWorkflowStatus(chID *iscp.ChainID) (*model.ConsensusWorkflowStatus, error) {
	...
	if err := c.do(http.MethodGet, routes.GetChainConsensusWorkflowStatus(chID.AsAddress().Bech32(iscp.NetworkPrefix)), nil, ncmm); err != nil {
		return nil, err
	}
	return ncmm, nil
}

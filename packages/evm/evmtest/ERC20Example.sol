...
contract ERC20Example {
    ...
    function mint(uint256 amount, uint64 storageDeposit) public {
      ISC.accounts.mintNativeTokens(msg.sender, foundrySN, amount, makeAllowanceBaseTokens(storageDeposit));
    }
    ...
    function createFoundry(uint256 maxSupply, uint64 storageDeposit) public {
      ...
      foundrySN = ISC.accounts.foundryCreateNew(msg.sender, tokenScheme, makeAllowanceBaseTokens(storageDeposit));
    }

    function registerToken(string memory name, string memory symbol, uint8 decimals, uint64 storageDeposit) public {
      ISC.sandbox.registerERC20NativeToken(msg.sender, foundrySN, name, symbol, decimals, makeAllowanceBaseTokens(storageDeposit));
    }

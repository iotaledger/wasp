...
func (c *Index) IndexBlock(trieRoot trie.Hash) {
	...
	nextBlockInfo, found := blocklog.NewStateReader(state).GetBlockInfo(blockIndexToCache + 1)
	if !found {
	...
	for i := blockIndexToCache; i >= cacheUntil; i-- {
		...
		blockinfo, found := blocklog.NewStateReader(activeStateToCache).GetBlockInfo(i)
		if !found {

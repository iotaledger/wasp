...
type Output struct {
	...
	NeedMempoolProposal       *isctypes.Anchor  // Requests for the mempool are needed for this Base Alias Output.
	NeedMempoolRequests       []*isc.RequestRef // Request payloads are needed from mempool for this IDs/Hash.
	NeedStateMgrStateProposal *isctypes.Anchor  // Query for a proposal for Virtual State (it will go to the batch proposal).
	NeedStateMgrDecidedState  *isctypes.Anchor  // Query for a decided Virtual State to be used by VM.
...
type Result struct {
	...
	NextAliasOutput *isctypes.Anchor             // AO produced in the TX.
...
func (c *consImpl) uponMPProposalInputsReady(baseAliasOutput *isctypes.Anchor) gpa.OutMessages {
...
func (c *consImpl) uponSMStateProposalQueryInputsReady(baseAliasOutput *isctypes.Anchor) gpa.OutMessages {
	c.output.NeedStateMgrStateProposal = baseAliasOutput
	return nil
}

func (c *consImpl) uponSMStateProposalReceived(proposedAliasOutput *isctypes.Anchor) gpa.OutMessages {
	c.output.NeedStateMgrStateProposal = nil
	return c.subACS.StateProposalReceived(proposedAliasOutput)
}

func (c *consImpl) uponSMDecidedStateQueryInputsReady(decidedBaseAliasOutput *isctypes.Anchor) gpa.OutMessages {
...
func (c *consImpl) uponACSInputsReceived(baseAliasOutput *isctypes.Anchor, requestRefs []*isc.RequestRef, dssIndexProposal []int, timeData time.Time) gpa.OutMessages {

...
func (bps batchProposalSet) decidedBaseAliasOutput(f int) *iscmove.AnchorWithRef {
	counts := map[hashing.HashValue]int{}
	values := map[hashing.HashValue]*iscmove.AnchorWithRef{}
	for _, bp := range bps {
	...
	var found *iscmove.AnchorWithRef
	var uncertain bool
...
func (bps batchProposalSet) decidedRequestRefs(f int, ao *iscmove.AnchorWithRef) []*isc.RequestRef {

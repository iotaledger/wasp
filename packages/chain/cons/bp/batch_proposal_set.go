...
func (bps batchProposalSet) decidedBaseAliasOutput(f int) *isctypes.Anchor {
	counts := map[hashing.HashValue]int{}
	values := map[hashing.HashValue]*isctypes.Anchor{}
	for _, bp := range bps {
...
func (bps batchProposalSet) decidedRequestRefs(f int, ao *isctypes.Anchor) []*isc.RequestRef {

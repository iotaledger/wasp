...
func testCommitteeLogBasic(t *testing.T, n, f int) {
	...
	for i := range gpaNodeIDs {
		...
		committeeLogInst, err := committeelog.New(gpaNodeIDs[i], chainID, distKeyPart, consensusStateRegistry, gpa.NodeIDFromPublicKey, true, -1, nil, log.NewChildLogger(fmt.Sprintf("N%v", i)))
		require.NoError(t, err)

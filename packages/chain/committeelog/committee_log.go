...
// ChainLog is the public interface for this algorithm.
...
var ErrCommitteeLogStateNotFound = errors.New("errCommitteeLogStateNotFound")
...
type committeeLogImpl struct {
	...
	logIndexVariable          LogIndexVariable          // Calculates the current log index.
	localViewVariable         LocalViewVariable         // Tracks the pending alias outputs.
	consensusInstanceVariable ConsensusInstanceVariable // The main algorithm.
...
func New(
	...
	cl := &committeeLogImpl{
		...
		consensusStateRegistry:    consensusStateRegistry,
		logIndexVariable:          nil, // Set bellow.
		localViewVariable:         nil, // Set bellow.
		consensusInstanceVariable: nil, // Set bellow.
		suspended:                 true,
		output:                    nil,
		first:                     true,
		log:                       log,
	...
	cl.consensusInstanceVariable = NewConsensusInstanceVariable(prevLI.Next(), persistLIFunc, func(out Output) {
		log.LogDebugf("ConsensusInstanceVariable: Output received, %v", out)
		cl.output = out
	}, log.NewChildLogger("VCI"))
	cl.logIndexVariable = NewLogIndexVariable(nodeIDs, n, f, prevLI, func(li LogIndex) gpa.OutMessages {
		log.LogDebugf("LogIndexVariable: Output received, %v", li)
		return cl.consensusInstanceVariable.LatestSeenLI(li, cl.logIndexVariable.ConsensusStarted)
	}, cclMetrics, log.NewChildLogger("VLI"))
	cl.localViewVariable = NewLocalViewVariable(pipeliningLimit, func(ao *isc.StateAnchor) gpa.OutMessages {
		log.LogDebugf("LocalViewVariable: Output received, %v", ao)
		return cl.consensusInstanceVariable.LatestL1Anchor(ao, cl.logIndexVariable.ConsensusStarted)
...
func (cl *committeeLogImpl) handleInputAnchorConfirmed(input *inputAnchorConfirmed) gpa.OutMessages {
	cl.suspended = false
	return cl.localViewVariable.AnchorConfirmed(input.anchor)
}
...
func (cl *committeeLogImpl) handleInputConsensusOutputSkip(input *inputConsensusOutputSkip) gpa.OutMessages {
	return cl.consensusInstanceVariable.ConsOutputSkip(input.logIndex, cl.logIndexVariable.ConsensusStarted)
}
...
func (cl *committeeLogImpl) handleInputConsensusOutputConfirmed(input *inputConsensusOutputConfirmed) gpa.OutMessages {
	return cl.consensusInstanceVariable.ConsOutputDone(input.logIndex, input.nextAnchor, cl.logIndexVariable.ConsensusStarted)
}
...
func (cl *committeeLogImpl) handleInputConsensusOutputRejected(input *inputConsensusOutputRejected) gpa.OutMessages {
	return cl.consensusInstanceVariable.ConsOutputSkip(input.logIndex, cl.logIndexVariable.ConsensusStarted) // This will cause proposal of our latest L1 Anchor.
}
...
func (cl *committeeLogImpl) handleInputConsensusTimeout(input *inputConsensusTimeout) gpa.OutMessages {
	return cl.consensusInstanceVariable.ConsTimeout(input.logIndex, cl.logIndexVariable.ConsensusStarted)
}

func (cl *committeeLogImpl) handleInputCanPropose() gpa.OutMessages {
	msgs := gpa.NoMessages()
	msgs.AddAll(cl.consensusInstanceVariable.Tick(cl.logIndexVariable.ConsensusStarted))
	...
	if cl.first && cl.output != nil && len(cl.output) > 0 {
		...
		for li := range cl.output {
			...
			msgs.AddAll(cl.logIndexVariable.ConsensusStarted(li))
		}
		return msgs
	}
	return msgs
...
func (cl *committeeLogImpl) handleMsgNextLogIndex(msg *MsgNextLogIndex) gpa.OutMessages {
	return cl.logIndexVariable.MsgNextLogIndexReceived(msg)
}
...
func (cl *committeeLogImpl) StatusString() string {
	return fmt.Sprintf(
		...
		cl.consensusInstanceVariable.StatusString(),
		cl.localViewVariable.StatusString(),
		cl.logIndexVariable.StatusString(),
	)
}

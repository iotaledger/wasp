---- MODULE WaspChainCommitteeLogSUI -------------------------------------------------
(******************************************************************************)
(*
...
The algorithm in general:
  ...
  - A node can join the LastLI after LastLI-1 is completed or a timeout exceeds.
    The node proposes input as follows:
      ...
      - If LastLI-1 resulted in Anchor_x, propose Anchor_x.
      - If LastLI-1 resulted in ⊥, propose latest Anchor from LocalView.
...
Notes on modeling:
  ...
  - To make state space a bit less, we assume the decided Anchors will come in some specific order.
...
ASSUME Cardinality(Anchor) >= Cardinality(LI)

VARIABLE l1Anchors      \* A chain of already committed Anchor objects in L1.
VARIABLE l1View     \* Latest Anchor (by position) seen by a node.
...
vars == <<l1Anchors, l1View, l2MinLI, l2LastLI, l2Input, l2Decision, l2Output>>

Node == AN

BOT         == 0            \* ⊥ as an Anchor.
Pending     == -1           \* Pending, not yet proposed Anchor.
SomeAoOrder == SetToSeq(Anchor) \* We don't need to consider all the permulations.

TypeOK ==
    /\ l1Anchors \in Seq(Anchor)
    /\ l1View \in [Node -> DOMAIN SomeAoOrder]
    /\ l2MinLI \in [Node -> LI]
    /\ l2LastLI \in [Node -> LI]
    /\ l2Input \in [Node -> [LI -> Anchor \cup {BOT, Pending}]]
    /\ l2Decision \in [LI -> Anchor \cup {BOT, Pending}]
    /\ l2Output \in [Node -> [LI -> Anchor \cup {BOT, Pending}]]
...
(*
Checks if next follows ao in the predefined order of Anchors.
*)
isNextAnchor(ao, next) ==
...
(*
Returns an Anchor which is reported as latest in the L1 at a particular node.
*)
lastSeenAnchor(n) == SomeAoOrder[l1View[n]]
...
propose(n, li, v) ==
    ...
    /\ UNCHANGED <<l1Anchors, l1View, l2MinLI, l2LastLI, l2Decision, l2Output>>
...
*)
AdvanceNodeView(n) ==
    \E i \in DOMAIN l1Anchors :
        /\ i > l1View[n]
        /\ l1View' = [l1View EXCEPT ![n] = i]
        /\ UNCHANGED <<l1Anchors, l2MinLI, l2LastLI, l2Input, l2Decision, l2Output>>
...
AdvanceLastLI(n) ==
    \E li \in LI :
        ...
        /\ UNCHANGED <<l1Anchors, l1View, l2MinLI, l2Input, l2Decision, l2Output>>
...
(*
When a node gets ⊥ output from LastLI-1, it provides own Anchor, as reported by the LocalView.
*)
ProposeOnPrevLIBot(n) ==
    LET li == l2LastLI[n]
    IN  /\ li > l2MinLI[n]                  \* MinLI will be handled by ProposeOnMinLI.
        /\ l2Output[n][li-1] = BOT          \* If we have ⊥ as an output from the previous LI.
        /\ propose(n, li, lastSeenAnchor(n))    \* Latest Anchor received from L1.
...
ProposeOnPrevLIOut(n) ==
    LET li == l2LastLI[n]
    IN  /\ li > l2MinLI[n]                      \* MinLI will be handled by ProposeOnMinLI.
        /\ l2Output[n][li-1] \in Anchor             \* If we have ⊥ as an output from the previous LI.
...
DecideOnL2 ==
    \E li \in LI:
        ...
        IN  /\  existQNF
            /\  \/  \E inpAnchor, outAnchor \in Anchor :
                        /\ existQ1N(inpAnchor)
                        /\ isNextAnchor(inpAnchor, outAnchor)
                        /\ l2Decision' = [l2Decision EXCEPT ![li] = outAnchor]
                \/  /\ ~\E ao \in Anchor : existQ1N(ao)
                    /\ l2Decision' = [l2Decision EXCEPT ![li] = BOT]
            /\ UNCHANGED <<l1Anchors, l1View, l2MinLI, l2LastLI, l2Input, l2Output>>
...
ObtainL2Output(n) ==
    \E li \in LI :
        ...
        /\ UNCHANGED <<l1Anchors, l1View, l2MinLI, l2Input, l2Decision>>


(*
A node publish its consensus output if it sees the TX input as the latest Anchor in L1.
*)
PublishL2Output(n) ==
    \E li \in LI:
        /\ isNextAnchor(lastSeenAnchor(n), l2Output[n][li])
        /\ l1View[n] = Len(l1Anchors)
        /\ l1Anchors' = Append(l1Anchors, l2Output[n][li])
...
DropOldInstances(n) ==
    ...
    /\ UNCHANGED <<l1Anchors, l1View, l2LastLI, l2Decision, l2Output>>
...
Reboot(n) ==
    \E li \in LI:
        ...
        /\ UNCHANGED <<l1Anchors, l1View, l2Decision, l2Output>>

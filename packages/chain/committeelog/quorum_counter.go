...
func (qc *QuorumCounter) VoteReceived(vote gpa.TypedMessageIn[*MsgNextLogIndex]) {
	sender := vote.Sender
	...
	if prevPeerLI.AsUint32() >= vote.Payload.NextLogIndex.AsUint32() {
		return
	}
	qc.maxPeerVotes[sender] = vote.Payload
}

...
// 		committeeLogInst, err := committeelog.New(gpaNodeIDs[i], sm.chainID, distKeyPart, consensusStateRegistry, gpa.NodeIDFromPublicKey, true, -1, 1, nil, log.NewChildLogger(fmt.Sprintf("N%v", i)))
// 		require.NoError(t, err)
...
// 	sm.tc.WithInput(nodeID, committeelog.NewInputAnchorConfirmed(ao))
// 	sm.tc.RunAll()
// }

...
type ConsensusInstanceVariable interface {
...
type consensusInstanceVariableImpl struct {
	haveConsOut bool
...
var _ ConsensusInstanceVariable = &consensusInstanceVariableImpl{}

// NewConsensusInstanceVariable is a constructor.
func NewConsensusInstanceVariable(
	minLI LogIndex,
	persistCB func(li LogIndex),
	outputCB func(lis Output),
	log log.Logger,
) ConsensusInstanceVariable {
	vci := &consensusInstanceVariableImpl{
		haveConsOut: false,
		lis: map[LogIndex]*isc.StateAnchor{
			minLI: nil,
		},
...
func (vci *consensusInstanceVariableImpl) ConsOutputDone(li LogIndex, producedAnchor *isc.StateAnchor, cb onLIInc) gpa.OutMessages {
	vci.haveConsOut = true
	return vci.trySet(li.Next(), producedAnchor, cb)
}

// Consensus at LI terminate with a SKIP/⊥ decision.
func (vci *consensusInstanceVariableImpl) ConsOutputSkip(li LogIndex, cb onLIInc) gpa.OutMessages {
	vci.haveConsOut = true
...
func (vci *consensusInstanceVariableImpl) ConsTimeout(li LogIndex, cb onLIInc) gpa.OutMessages {
	return vci.trySet(li.Next(), nil, cb)
}

// If we see consensus proposals from F+1 nodes at seenLI...
func (vci *consensusInstanceVariableImpl) LatestSeenLI(seenLI LogIndex, cb onLIInc) gpa.OutMessages {
	msgs := gpa.NoMessages()
	msgs.AddAll(vci.trySet(seenLI.Prev(), nil, cb))
	if !vci.haveConsOut {
		// Still don't have the initial round succeeded, thus keep proposing the NIL.
		// A race condition is possible between receiving the next LI from the LogIndexVariable,
		// and receiving the consensus output. While the actual convergence at runtime
		// happens anyway, we delay reaction to the LogIndexVariable output to some delay to make
...
func (vci *consensusInstanceVariableImpl) LatestL1Anchor(ao *isc.StateAnchor, cb onLIInc) gpa.OutMessages {
	vci.lastAnchor = ao
	return vci.trySet(vci.lastLI, ao, cb) // Finish ConsOutputSkipBase, if pending.
}

func (vci *consensusInstanceVariableImpl) Tick(cb onLIInc) gpa.OutMessages {
	n := len(vci.delayed)
...
func (vci *consensusInstanceVariableImpl) trySet(li LogIndex, ao *isc.StateAnchor, cb onLIInc) gpa.OutMessages {
	//
	// Is it outdated?
	if li < vci.minLI {
		return nil
...
func (vci *consensusInstanceVariableImpl) StatusString() string {
	buf := ""
	for li := vci.minLI; li <= vci.maxLI; li = li.Next() {
		ao, ok := vci.lis[li]
		if !ok {
	...
	return fmt.Sprintf("{consensusInstanceVariable: minLI=%v,%s}", vci.minLI, buf)
}

...
func TestLogIndexVariableV2Basic(t *testing.T) {
	...
	vli := committeelog.NewLogIndexVariable(nodeIDs, n, f, initLI, func(li committeelog.LogIndex) gpa.OutMessages {
		vliOut = li
		return nil
	}, nil, log)
	//
...
func TestLogIndexVariableV2Other(t *testing.T) {
	log := testlogger.NewLogger(t)
	defer log.Shutdown()
	n := 4
	f := 1

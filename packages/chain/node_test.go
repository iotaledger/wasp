...
func (tnc *testNodeConn) L1ParamsFetcher() param_fetcher.L1ParamsFetcher {
...
func newTestNodeConn(t *testing.T, l1Client clients.L1Client, iscPackageID iotago.PackageID) *testNodeConn {
	tnc := &testNodeConn{
		...
		l1ParamsFetcher: param_fetcher.NewL1ParamsFetcher(l1Client, log.EmptyLogger),
	}
	tnc.attachWG.Add(1)
	return tnc
}
...
func (tnc *testNodeConn) ConsensusL1InfoProposal(
	...
	t := make(chan consensusrunner.NodeConnL1Info)
...
func newEnv(t *testing.T, n, f int, reliable bool, node l1starter.IotaNodeEndpoint) *testEnv {
	...
	for i := range te.peerIdentities {
		...
		te.nodes[i], err = chain.New(
			...
			log,
			te.chainID,
			indexedstore.NewFake(statetest.NewStoreWithUniqueWriteMutex(mapdb.NewMapDB())),
			te.nodeConns[i],
			te.peerIdentities[i],

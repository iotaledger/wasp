...
type StateTracker interface {
	...
	TrackAliasOutput(ao *isctypes.Anchor, strict bool)
...
type StateTrackerStepCB = func(st state.State, from, till *isctypes.Anchor, added, removed []state.Block)
...
type stateTrackerImpl struct {
	...
	haveAO                 *isctypes.Anchor   // We have a state ready for this AO.
	nextAO                 *isctypes.Anchor   // For this state a query was made, but the response not received yet.
...
func (sti *stateTrackerImpl) TrackAliasOutput(ao *isctypes.Anchor, strict bool) {

...
func TestWriteReadDifferentStores(t *testing.T) {
	...
	err = writeSnapshotToFile(origSnapshot, fileName, f)
	require.NoError(t, err)
	newSnapshot, err := readSnapshotFromFile(fileName)
	require.NoError(t, err)
	err = os.Remove(fileName)
	require.NoError(t, err)

	store := state.NewStore(mapdb.NewMapDB())
	err = store.RestoreSnapshot(lastCommitment.TrieRoot(), newSnapshot)
	require.NoError(t, err)

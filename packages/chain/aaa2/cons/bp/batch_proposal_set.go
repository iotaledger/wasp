...
func (bps batchProposalSet) decidedBaseAliasOutputID(f int) *iotago.OutputID {
	aos := map[iotago.OutputID]int{}
	for _, bp := range bps {
		aos[bp.stateOutputID.ID()]++
	}
	var found *iotago.OutputID
	for aoID, count := range aos {
		if count > f {
			if found != nil {

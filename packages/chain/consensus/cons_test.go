...
func testConsBasic(t *testing.T, n, f int) {
	...
	committeeAddress, distKeyPartProviders := testpeers.SetupDkgTrivial(t, n, f, peerIdentities, nil)
	...
	for i, nid := range nodeIDs {
		...
		nodeDistKeyPart, err := distKeyPartProviders[i].LoadDistKeyPart(committeeAddress)
		require.NoError(t, err)
...
func testChained(t *testing.T, n, f, b int) {
	...
	for i := range testChainInsts {
		...
		testChainInsts[i] = *newTestConsInst(
			...
			testNodeStates, peerIdentities, distKeyPartProviders,
...
func newTestConsInst(
	...
	distKeyPartRegistryProviders []registry.DistKeyPartRegistryProvider,
	requests []isc.Request,
	doneCB func(nextInput *testInstInput),
	log log.Logger,
) *testConsInst {
	...
	for i, nid := range nodeIDs {
		nodeLog := log.NewChildLogger(nid.ShortString())
		nodeSK := peerIdentities[i].GetPrivateKey()
		nodeDistKeyPart, err := distKeyPartRegistryProviders[i].LoadDistKeyPart(committeeAddress)
		require.NoError(t, err)
		nodes[nid] = cons.New(chainID, nodeStates[nid], nid, nodeSK, nodeDistKeyPart, procCache, consInstID, gpa.NodeIDFromPublicKey, accounts.CommonAccount(), nodeLog).AsGPA()
	}
	tci := &testConsInst{
		t:                                t,
		nodes:                            nodes,

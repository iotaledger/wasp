...
func NewNode(env *MockedEnv, nodeIndex uint16, timers ConsensusTimers) *mockedNode { //nolint:revive
	...
	require.NoError(env.T, err)
	cons := New(ret.ChainCore, ret.Mempool, cmt, cmtPeerGroup, chainNodeConn, true, nil, metrics.DefaultChainMetrics(), wal.NewDefault(), timers)
	...
	ret.Log.Debugf("Mocked node %v started: id %v public key %v", ret.NodeIndex, ret.NodeID, ret.NodePubKey.AsString())
	return ret
}

...
func testGrBasic(t *testing.T, n, f int, reliable bool) {
	...
	err := l1starter.Instance().L1Client().RequestFundsFromFaucet(ctx, originator.Address().AsIotaAddress())
	...
	outputChs := make([]chan *consensusrunner.Output, len(nodes))
	for i, n := range nodes {
		outputCh := make(chan *consensusrunner.Output, 1)
		outputChs[i] = outputCh
		n.Input(anchor, func(o *consensusrunner.Output) { outputCh <- o }, func() {})
	}
	...
	//
	// Wait for outputs.
	var firstOutput *consensusrunner.Output
...
func (tmp *testMempool) ConsensusProposalAsync(ctx context.Context, anchor *isc.StateAnchor, consensusID consensusrunner.ConsensusID) <-chan []*isc.RequestRef {
	tmp.lock.Lock()
	defer tmp.lock.Unlock()
...
var _ consensusrunner.NodeConn = &testNodeConn{}

func newTestNodeConn(gasCoin *coin.CoinWithRef) *testNodeConn {
	return &testNodeConn{gasCoin: gasCoin}
}

func (t *testNodeConn) ConsensusL1InfoProposal(ctx context.Context, anchor *isc.StateAnchor) <-chan consensusrunner.NodeConnL1Info {
	ch := make(chan consensusrunner.NodeConnL1Info, 1)

...
type SyncVM interface {
	DecidedBatchProposalsReceived(aggregatedProposals *batch_proposal.AggregatedBatchProposals) gpa.OutMessages
...
type syncVMImpl struct {
	aggregatedProposals *batch_proposal.AggregatedBatchProposals
	chainState          state.State
	randomness          *hashing.HashValue
	requests            []isc.Request
	vmResult            *vm.VMTaskResult
	inputsReady         bool
	inputsReadyCB       func(aggregatedProposals *batch_proposal.AggregatedBatchProposals, chainState state.State, randomness *hashing.HashValue, requests []isc.Request) gpa.OutMessages
	outputReady         bool
	outputReadyCB       func(output *vm.VMTaskResult, aggregatedProposals *batch_proposal.AggregatedBatchProposals) gpa.OutMessages
}
...
func (sub *syncVMImpl) DecidedBatchProposalsReceived(aggregatedProposals *batch_proposal.AggregatedBatchProposals) gpa.OutMessages {

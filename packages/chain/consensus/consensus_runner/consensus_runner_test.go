...
func testGrBasic(t *testing.T, n, f int, reliable bool) {
	...
	nodes := make([]*consGR.ConsensusRunner, len(peerIdentities))
	...
	for i := range peerIdentities {
		distKeyPart, err := distKeyPartProviders[i].LoadDistKeyPart(committeeAddress)
		require.NoError(t, err)

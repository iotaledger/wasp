...
// Same as TestBasic, just with unreliable network and ack-handler.
	...
	test := func(tt *testing.T, n, f int) {
		...
		for {
			...
			if predicate() {
				...
				for _, nid := range nodeIDs {
					tc.WithInput(nid, gpas[nid].(gpa.AckHandler).MakeTickInput(tick))
				}
				log.LogDebugf("Doing a message redelivery.")
			}
		}
		//
		// Check the INTERMEDIATE result.
		...
		tc.WithInputProbability(0.001)
		for {
			allNodesHaveSigPredicate := func() bool {
				for _, n := range gpas {
					o := n.Output()
					if o == nil || o.(*dss.Output).Signature == nil {
						return false
					}
					require.NotNil(tt, o)
				}
			...
			tc.RunUntil(allNodesHaveSigPredicate)
			if allNodesHaveSigPredicate() {
				break
			} else {
				tick = tick.Add(time.Second)
				for _, nid := range nodeIDs {

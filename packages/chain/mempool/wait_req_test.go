...
func TestWaitReq(t *testing.T) {
	...
	req0 := isc.NewOffLedgerRequest(isc.RandomChainID(), isc.NewMessage(isc.Hn("foo"), isc.Hn("bar"), nil), 0, gas.LimitsDefault.MaxGasPerRequest).Sign(kp)
	req1 := isc.NewOffLedgerRequest(isc.RandomChainID(), isc.NewMessage(isc.Hn("foo"), isc.Hn("bar"), nil), 1, gas.LimitsDefault.MaxGasPerRequest).Sign(kp)
	req2 := isc.NewOffLedgerRequest(isc.RandomChainID(), isc.NewMessage(isc.Hn("foo"), isc.Hn("bar"), nil), 2, gas.LimitsDefault.MaxGasPerRequest).Sign(kp)

...
type mempool struct {
	inBuffer                map[iscp.RequestID]iscp.RequestRaw
...
func New(stateReader state.OptimisticStateReader, log *logger.Logger, mempoolMetrics metrics.MempoolMetrics, solidificationLoopDelay ...time.Duration) Mempool {
	ret := &mempool{
		inBuffer:       make(map[iscp.RequestID]iscp.RequestRaw),
...
func (m *mempool) addToInBuffer(req iscp.RequestRaw) bool {
...
func (m *mempool) takeInBuffer(buf []iscp.RequestRaw) []iscp.RequestRaw {
	buf = buf[:0]
	m.inMutex.RLock()
...
func (m *mempool) addToPool(req iscp.RequestRaw) bool {
	reqid := req.ID()
...
func (m *mempool) countRequestInMetrics(req iscp.RequestRaw) {
...
func (m *mempool) ReceiveRequests(reqs ...iscp.RequestRaw) {
...
func (m *mempool) ReceiveRequest(req iscp.RequestRaw) bool {
	m.countRequestInMetrics(req)
...
func (m *mempool) traceIn(req iscp.RequestRaw) {
	rotateStr := ""
...
func (m *mempool) ReadyNow(currentTime ...time.Time) []iscp.RequestRaw {
	m.poolMutex.RLock()
	...
	var oldestRotate iscp.RequestRaw
	var oldestRotateTime time.Time

	toRemove := []iscp.RequestID{}

	ret := make([]iscp.RequestRaw, 0, len(m.pool))
	...
	if oldestRotate != nil {
		return []iscp.RequestRaw{oldestRotate}
	}
	return ret
}
...
func (m *mempool) ReadyFromIDs(currentTime time.Time, reqIDs ...iscp.RequestID) ([]iscp.RequestRaw, []int, bool) {
	requests := make([]iscp.RequestRaw, 0, len(reqIDs))
...
func (m *mempool) GetRequest(id iscp.RequestID) iscp.RequestRaw {
	m.poolMutex.RLock()
	defer m.poolMutex.RUnlock()
...
func (m *mempool) moveToPoolLoop() {
	buf := make([]iscp.RequestRaw, 0, 100)
	for {
		select {
		case <-m.chStop:
			return

...
type Mempool interface {
	...
	TrackNewChainHead(st state.State, from, till *iscmove_types.Anchor, added, removed []state.Block) <-chan bool
...
type mempoolImpl struct {
	...
	distSync                       gpa.GPA
	chainHeadAO                    *iscmove_types.Anchor
...
type reqTrackNewChainHead struct {
	st         state.State
	from       *iscmove_types.Anchor
	till       *iscmove_types.Anchor
	added      []state.Block
	removed    []state.Block
...
func (mpi *mempoolImpl) TrackNewChainHead(st state.State, from, till *iscmove_types.Anchor, added, removed []state.Block) <-chan bool {
...
func (mpi *mempoolImpl) ConsensusProposalAsync(ctx context.Context, aliasOutput *iscmove_types.Anchor, consensusID consGR.ConsensusID) <-chan []*isc.RequestRef {

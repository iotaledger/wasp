...
Msgs == [
    ...
    chains: SUBSET Chains \* Access to these chains is granted by src to dst.
]
...
accessToChains(n, m) == {c \in Chains : c \in active[n] /\ m \in access[n][c]}
accessMsgs(n, q) == {[
        t      |-> "ACCESS",
        src    |-> n,
        dst    |-> dst,
        src_lc |-> lClock'[n][n],
        dst_lc |-> IF q THEN 0 ELSE lClock'[n][dst],
        chains |-> accessToChains(n, dst)'
    ] : dst \in (Nodes \ {n})}
...
ChainActivate(n, c) ==
    ...
    /\ msgs' = msgs \cup accessMsgs(n, TRUE)
...
ChainDeactivate(n, c) ==
    ...
    /\ msgs' = msgs \cup accessMsgs(n, FALSE)
...
AccessNodeAdd(n, c, a) ==
    ...
    /\ \/ c \in    active[n] /\ msgs' = msgs \cup accessMsgs(n, FALSE)
...
RecvAccess(n) == \E m \in msgs:
    \* /\ LC_HaveNext(n, 1)
    /\ m.t = "ACCESS" /\ m.dst = n
    /\ lClock' = [lClock EXCEPT ![n][n]     = IF @ > m.dst_lc THEN @ ELSE m.dst_lc, \* TODO: + 1,
    ...
    /\ \/ /\ m.src_lc >= lClock[n][m.src] \* Update the servers, if message not outdated.
          /\ server' = [server EXCEPT ![n] = [c \in Chains |-> IF c \in m.chains
                                                               THEN server[n][c] \cup {m.src}
                                                               ELSE server[n][c] \ {m.src} ]]
       \/ /\ m.src_lc < lClock[n][m.src]
          /\ UNCHANGED <<server>>
    /\ \/ m.dst_lc <  lClock[n][n] /\ sendMaybeAck(m, accessMsgs(n, FALSE))
       \/ m.dst_lc >= lClock[n][n] /\ sendMaybeAck(m, {})

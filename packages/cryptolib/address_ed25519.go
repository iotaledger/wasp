...
func NewEd25519AddressFromBytes(bytes []byte) (*Ed25519Address, error) {
	if len(bytes) != Ed25519AddressBytesLength {
		return nil, fmt.Errorf("address must be %d bytes long but is %d", Ed25519AddressBytesLength, len(bytes))
	}
...
func (edAddr *Ed25519Address) Size() int {
	return Ed25519AddressBytesLength
}

func (edAddr *Ed25519Address) Bytes() []byte {
	result := make([]byte, Ed25519AddressBytesLength)
	copy(result[:], edAddr[:])
	return result
}
...
/* func (edAddr *Ed25519Address) Bech32(hrp NetworkPrefix) string {
	return addressToBech32String(hrp, edAddr)
}*/

...
func NewAliasAddressFromBytes(bytes []byte) (*AliasAddress, error) {
	if len(bytes) != AliasAddressBytesLength {
		return nil, fmt.Errorf("address must be %d bytes long but is %d", AliasAddressBytesLength, len(bytes))
	}
	result := AliasAddress{}
	copy(result[:], bytes[:])
	return &result, nil
...
func (aliasAddr *AliasAddress) Size() int {
	return AliasAddressBytesLength
}

func (aliasAddr *AliasAddress) Bytes() []byte {
	result := make([]byte, AliasAddressBytesLength)
	copy(result[:], aliasAddr[:])
	return result
}
...
/* func (aliasAddr *AliasAddress) Bech32(hrp NetworkPrefix) string {
	return addressToBech32String(hrp, aliasAddr)
}*/

...
type CommitteeService struct {
	...
	distKeysPartRegistry registry.DistKeyPartsRegistry
}

func NewCommitteeService(chainsProvider chains.Provider, networkProvider peering.NetworkProvider, distKeyPartsRegistry registry.DistKeyPartsRegistry) interfaces.CommitteeService {
	return &CommitteeService{
		chainsProvider:       chainsProvider,
		networkProvider:      networkProvider,
		distKeysPartRegistry: distKeyPartsRegistry,
	}
}
...
func (c *CommitteeService) GetCommitteeInfo(chainID isc.ChainID) (*dto.ChainNodeInfo, error) {
	...
	distKeyPart, err := c.distKeysPartRegistry.LoadDistKeyPart(committeeInfo.Address)
	if err != nil {
		return nil, err
	}
...
func (c *CommitteeService) getCommitteeNodes(
	...
	for _, committeeNodePubKey := range distKeyPart.GetNodePubKeys() {

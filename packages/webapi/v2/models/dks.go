...
type DKSharesPostRequest struct {
	PeerPublicKeys []string `json:"peerPublicKeys" swagger:"desc(Optional, Hex encoded public keys of the peers generating the DKS.)"`
...
type DKSharesInfo struct {
	...
	PeerPublicKeys [][]byte `json:"peerPublicKeys" swagger:"desc(Public keys of the nodes sharing the key.)"`

...
// TestBasic checks if DistKeyGeneration procedure is executed successfully in a common case.
func TestBasic(t *testing.T) {
	...
	var aggrDks tcrypto.DistibutedKeyPart
	for i, r := range distKeyPartRegistryProviders {
...
// TestUnreliableNet checks, if DistKeyGeneration runs on an unreliable network.
...
// TestLowN checks, if the DistKeyGeneration works with N=1 and other low values. N=1 is a special case.
func TestLowN(t *testing.T) {
	...
	for n := uint16(1); n < 4; n++ {
		t.Logf("------------------> running DistKeyGeneration with %v", n)
		timeout := 100 * time.Second
		threshold := n
		peerCount := n
		peerPeeringURLs, peerIdentities := testpeers.SetupKeys(peerCount)

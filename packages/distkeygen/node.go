...
// Node represents a node, that can participate in a DistKeyGeneration procedure.
// It receives commands from the initiator as a distKeyGen.NodeProvider,
// and communicates with other DistKeyGeneration nodes via the peering network.
type Node struct {
	...
	distKeyPartRegistryProvider registry.DistKeyPartRegistryProvider      // Where to store the generated keys.
...
// NewNode creates new node, that can participate in the DistKeyGeneration procedure.
// The node then can run several DistKeyGeneration procedures.
func NewNode(
...
func (n *Node) receiveInitMessage(peerMsg *peering.PeerMessageIn) {
	if peerMsg.MsgReceiver != peering.ReceiverDkgInit {
		panic(fmt.Errorf("DistKeyGeneration init handler does not accept peer messages of other receiver type %v, message type=%v",
			peerMsg.MsgReceiver, peerMsg.MsgType))
	}
	if peerMsg.MsgType != initiatorInitMsgType {
		panic(fmt.Errorf("wrong type of DistKeyGeneration init message: %v", peerMsg.MsgType))
	}
...
// GenerateDistributedKey takes all the required parameters from the node and initiated the DistKeyGeneration procedure.
// This function is executed on the DistKeyGeneration initiator node (a chosen leader for this DistKeyGeneration instance).
//
...
func (n *Node) GenerateDistributedKey(
...
) (tcrypto.DistibutedKeyPart, error) {
	n.log.LogInfof("Starting new DistKeyGeneration procedure, initiator=%v, peers=%+v", n.netProvider.Self().PeeringURL(), peerPubs)
	var err error
	...
	if peerCount < 1 || threshold < 1 || threshold > peerCount {
		return nil, invalidParams(fmt.Errorf("wrong DistKeyGeneration parameters: N = %d, T = %d", peerCount, threshold))
	}
	...
	if threshold < minQuorum {
		return nil, invalidParams(fmt.Errorf("wrong DistKeyGeneration parameters: for N = %d value T must be at least %d", peerCount, peerCount/2+1))
	}
	//
	// Setup network connections.
	distKeyGenID := peering.RandomPeeringID()
	var netGroup peering.GroupProvider
	if netGroup, err = n.netProvider.PeerGroup(distKeyGenID, peerPubs); err != nil {
		return nil, err
	}
	defer netGroup.Close()
	recvCh := make(chan *peering.PeerMessageIn, peerCount*2)
	unhook := n.netProvider.Attach(&distKeyGenID, peering.ReceiverDkg, func(recv *peering.PeerMessageIn) {
		recvCh <- recv
	})
	...
	if err = n.exchangeInitiatorAcks(netGroup, netGroup.AllNodes(), recvCh, rTimeout, gTimeout, rabinStep0Initialize,
		func(peerIdx uint16, peer peering.PeerSender) {
			...
			peer.SendMsg(makePeerMessage(initPeeringID, peering.ReceiverDkgInit, rabinStep0Initialize, &initiatorInitMsg{
				distKeyGenRef: distKeyGenID.String(), // It could be some other identifier.
				peeringID:     distKeyGenID,
				peerPubs:      peerPubs,
	...
	//
	// Perform the DistKeyGeneration steps, each step in parallel, all steps sequentially.
	// Step numbering (R) is according to <https://github.com/dedis/kyber/blob/master/share/dkg/rabin/distKeyGen.go>.
	if peerCount > 1 {
		if err = n.exchangeInitiatorStep(netGroup, netGroup.AllNodes(), recvCh, rTimeout, gTimeout, distKeyGenID, rabinStep1R21SendDeals); err != nil {
			return nil, err
		}
		if err = n.exchangeInitiatorStep(netGroup, netGroup.AllNodes(), recvCh, rTimeout, gTimeout, distKeyGenID, rabinStep2R22SendResponses); err != nil {
			return nil, err
		}
		if err = n.exchangeInitiatorStep(netGroup, netGroup.AllNodes(), recvCh, rTimeout, gTimeout, distKeyGenID, rabinStep3R23SendJustifications); err != nil {
			return nil, err
		}
		if err = n.exchangeInitiatorStep(netGroup, netGroup.AllNodes(), recvCh, rTimeout, gTimeout, distKeyGenID, rabinStep4R4SendSecretCommits); err != nil {
			return nil, err
		}
		if err = n.exchangeInitiatorStep(netGroup, netGroup.AllNodes(), recvCh, rTimeout, gTimeout, distKeyGenID, rabinStep5R5SendComplaintCommits); err != nil {
			return nil, err
		}
	}
	//
	...
	if err = n.exchangeInitiatorMsgs(netGroup, netGroup.AllNodes(), recvCh, rTimeout, gTimeout, rabinStep6R6SendReconstructCommits,
		func(peerIdx uint16, peer peering.PeerSender) {
			...
			peer.SendMsg(makePeerMessage(distKeyGenID, peering.ReceiverDkg, rabinStep6R6SendReconstructCommits, &initiatorStepMsg{}))
		},
	...
	if err = n.exchangeInitiatorAcks(netGroup, netGroup.AllNodes(), recvCh, rTimeout, gTimeout, rabinStep7CommitAndTerminate,
		func(peerIdx uint16, peer peering.PeerSender) {
			...
			peer.SendMsg(makePeerMessage(distKeyGenID, peering.ReceiverDkg, rabinStep7CommitAndTerminate, &initiatorDoneMsg{
...
// onInitMsg is a callback to handle the DistKeyGeneration initialization messages.
func (n *Node) onInitMsg(msg *initiatorInitMsgIn) {
	var err error
	var p *proc
	n.procLock.RLock()
	if n.processes.Has(msg.distKeyGenRef) {
	...
	go func() {
		...
		if p, err = onInitiatorInit(msg.peeringID, &msg.initiatorInitMsg, n); err == nil {
			n.processes.Set(p.distKeyGenRef, p)
		}
		n.procLock.Unlock()
		n.netProvider.SendMsgByPubKey(msg.SenderPubKey, makePeerMessage(msg.peeringID, peering.ReceiverDkg, msg.step, &initiatorStatusMsg{
			error: err,
...
// Called by the DistKeyGeneration process on termination.
func (n *Node) dropProcess(p *proc) bool {
	n.procLock.Lock()
	defer n.procLock.Unlock()

	return n.processes.Delete(p.distKeyGenRef)
}
...
func (n *Node) exchangeInitiatorStep(
	...
	distKeyGenID peering.PeeringID,
	step byte,
) error {
	sendCB := func(peerIdx uint16, peer peering.PeerSender) {
		...
		peer.SendMsg(makePeerMessage(distKeyGenID, peering.ReceiverDkg, step, &initiatorStepMsg{}))
	}

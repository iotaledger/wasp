...
// Stands for a DistKeyGeneration procedure instance on a particular node.
type proc struct {
	distKeyGenRef  string                    // User supplied unique ID for this instance.
	distKeyGenID   peering.PeeringID         // DistKeyGeneration procedure ID we are participating in.
	distKeyPart    tcrypto.DistibutedKeyPart // This will be generated as a result of this procedure.
	node           *Node                     // DistKeyGeneration node we are running in.
	...
	distKeyGenImpl map[keySetType]*rabin_dkg.DistKeyGenerator // The cryptographic implementation to use.
	distKeyGenLock *sync.RWMutex                              // Guard access to distKeyGenImpl
...
//nolint:funlen
func onInitiatorInit(distKeyGenID peering.PeeringID, msg *initiatorInitMsg, node *Node) (*proc, error) {
	log := node.log.NewChildLogger(fmt.Sprintf("distKeyGenID.%s", distKeyGenID.String()))
	var err error

	var netGroup peering.GroupProvider
	if netGroup, err = node.netProvider.PeerGroup(distKeyGenID, msg.peerPubs); err != nil {
		return nil, err
	}

	blsThreshold := deriveBlsThreshold(msg)

	var distKeyGenImpl map[keySetType]*rabin_dkg.DistKeyGenerator
	if len(msg.peerPubs) >= 2 {
		// We use real DistKeyGeneration only if N >= 2. Otherwise, we just generate key pair, and that's all.
		distKeyGenImpl = make(map[keySetType]*rabin_dkg.DistKeyGenerator)
		...
		if distKeyGenImpl[keySetTypeEd25519], err = rabin_dkg.NewDistKeyGenerator(node.edSuite, node.edSuite, node.secKey, kyberPeerPubs, int(msg.threshold)); err != nil {
			return nil, fmt.Errorf("failed to instantiate DistKeyGenerator: %w", err)
		}
		if distKeyGenImpl[keySetTypeBLS], err = rabin_dkg.NewDistKeyGenerator(node.blsSuite, node.edSuite, node.secKey, kyberPeerPubs, blsThreshold); err != nil {
			return nil, fmt.Errorf("failed to instantiate DistKeyGenerator: %w", err)
		}
	}
	...
	p := proc{
		distKeyGenRef:  msg.distKeyGenRef,
		distKeyGenID:   distKeyGenID,
		node:           node,
	...
	p.log.LogInfof("Starting DistKeyGeneration Peer process at %v for DkgID=%v", p.myPubKey.String(), p.distKeyGenID.String())
	stepsStart := make(chan multiKeySetMsgs)
	p.steps = make(map[byte]*procStep)
	if p.distKeyGenImpl == nil {
...
func (p *proc) rabinStep1R21SendDealsMakeSent(step byte, kst keySetType, initRecv *peering.PeerMessageGroupIn, prevMsgs map[uint16]*peering.PeerMessageData) (map[uint16]*peering.PeerMessageData, error) {
	...
	if deals, err = p.distKeyGenImpl[kst].Deals(); err != nil {
	...
	for i := range deals {
		...
		sentMsgs[iterator] = makePeerMessage(p.distKeyGenID, peering.ReceiverDkg, step, &rabinDealMsg{
			deal: deals[i],
		})
	}
	return sentMsgs, nil
...
func (p *proc) rabinStep1R21SendDealsMakeResp(step byte, initRecv *peering.PeerMessageGroupIn, recvMsgs multiKeySetMsgs) (*peering.PeerMessageData, error) {
	return makePeerMessage(p.distKeyGenID, peering.ReceiverDkg, step, &initiatorStatusMsg{error: nil}), nil
}
...
func (p *proc) rabinStep2R22SendResponsesMakeSent(step byte, kst keySetType, initRecv *peering.PeerMessageGroupIn, prevMsgs map[uint16]*peering.PeerMessageData) (map[uint16]*peering.PeerMessageData, error) {
	...
	for i := range recvDeals {
		...
		p.distKeyGenLock.Lock()
		if r, err = p.distKeyGenImpl[kst].ProcessDeal(recvDeals[i].deal); err != nil {
			p.distKeyGenLock.Unlock()
			p.log.LogErrorf("ProcessDeal(%v) -> %+v", i, err)
			return nil, err
		}
		p.distKeyGenLock.Unlock()
		p.log.LogDebugf("RabinDistKeyGeneration[%v] DealResponse[%v|%v]=%v", p.myPubKey.String(), r.Index, r.Response.Index, hexutil.Encode(r.Response.SessionID))
	...
	for i := range prevMsgs { // Use peerIdx from the previous round.
		sentMsgs[i] = makePeerMessage(p.distKeyGenID, peering.ReceiverDkg, step, &rabinResponseMsg{
...
func (p *proc) rabinStep3R23SendJustificationsMakeSent(step byte, kst keySetType, initRecv *peering.PeerMessageGroupIn, prevMsgs map[uint16]*peering.PeerMessageData) (map[uint16]*peering.PeerMessageData, error) {
	...
	for i := range recvResponses {
		for _, r := range recvResponses[i].responses {
			...
			p.log.LogDebugf("RabinDistKeyGeneration[%v] ProcResponse[%v|%v]=%v", p.myPubKey.String(), r.Index, r.Response.Index, hexutil.Encode(r.Response.SessionID))
			if j, err = p.distKeyGenImpl[kst].ProcessResponse(r); err != nil {
				p.distKeyGenLock.Unlock()
	...
	sentMsgs := make(map[uint16]*peering.PeerMessageData)
	for i := range prevMsgs { // Use peerIdx from the previous round.
		sentMsgs[i] = makePeerMessage(p.distKeyGenID, peering.ReceiverDkg, step, &rabinJustificationMsg{
...
func (p *proc) rabinStep4R4SendSecretCommitsMakeSent(step byte, kst keySetType, initRecv *peering.PeerMessageGroupIn, prevMsgs map[uint16]*peering.PeerMessageData) (map[uint16]*peering.PeerMessageData, error) {
	...
	for i := range recvJustifications {
		for _, j := range recvJustifications[i].justifications {
			if err = p.distKeyGenImpl[kst].ProcessJustification(j); err != nil {
				p.distKeyGenLock.Unlock()
	...
	// Take the QUAL set.
	p.distKeyGenLock.Lock()
	p.distKeyGenImpl[kst].SetTimeout()
	if !p.distKeyGenImpl[kst].Certified() {
		p.distKeyGenLock.Unlock()
	...
	if thisInQual {
		p.distKeyGenLock.Lock()
		if ourSecretCommits, err = p.distKeyGenImpl[kst].SecretCommits(); err != nil {
			p.distKeyGenLock.Unlock()
	...
	for i := range prevMsgs { // Use peerIdx from the previous round.
		if thisInQual && p.nodeInQUAL(kst, i) {
			sentMsgs[i] = makePeerMessage(p.distKeyGenID, peering.ReceiverDkg, step, &rabinSecretCommitsMsg{
...
func (p *proc) rabinStep5R5SendComplaintCommitsMakeSent(step byte, kst keySetType, initRecv *peering.PeerMessageGroupIn, prevMsgs map[uint16]*peering.PeerMessageData) (map[uint16]*peering.PeerMessageData, error) {
	...
	if p.nodeInQUAL(kst, p.nodeIndex) {
		for i := range recvSecretCommits {
			...
			if sc != nil {
				...
				if cc, err = p.distKeyGenImpl[kst].ProcessSecretCommits(sc); err != nil {
					p.distKeyGenLock.Unlock()
					return nil, err
				}
				p.distKeyGenLock.Unlock()
	...
	for i := range prevMsgs { // Use peerIdx from the previous round.
		if p.nodeInQUAL(kst, i) {
			sentMsgs[i] = makePeerMessage(p.distKeyGenID, peering.ReceiverDkg, step, &rabinComplaintCommitsMsg{
...
func (p *proc) rabinStep6R6SendReconstructCommitsMakeSent(step byte, kst keySetType, initRecv *peering.PeerMessageGroupIn, prevMsgs map[uint16]*peering.PeerMessageData) (map[uint16]*peering.PeerMessageData, error) {
	...
	if p.nodeInQUAL(kst, p.nodeIndex) {
		for i := range recvComplaintCommits {
			for _, cc := range recvComplaintCommits[i].complaintCommits {
				...
				if rc, err = p.distKeyGenImpl[kst].ProcessComplaintCommits(cc); err != nil {
					p.distKeyGenLock.Unlock()
					return nil, err
				}
				p.distKeyGenLock.Unlock()
	...
	for i := range prevMsgs { // Use peerIdx from the previous round.
		...
		sentMsgs[i] = makePeerMessage(p.distKeyGenID, peering.ReceiverDkg, step, msg)
	}
	return sentMsgs, nil
}
...
func (p *proc) rabinStep6R6SendReconstructCommitsMakeResp(
	...
	if p.distKeyGenImpl == nil {
		...
		for _, recvMsg := range recvMsgs {
			...
			for _, rc := range peerReconstructCommitsMsgEd.reconstructCommits {
				if err = p.distKeyGenImpl[keySetTypeEd25519].ProcessReconstructCommits(rc); err != nil {
					p.distKeyGenLock.Unlock()
					return nil, err
				}
			}
			for _, rc := range peerReconstructCommitsMsgBLS.reconstructCommits {
				if err = p.distKeyGenImpl[keySetTypeBLS].ProcessReconstructCommits(rc); err != nil {
					p.distKeyGenLock.Unlock()
					return nil, err
				}
			}
		...
		p.distKeyGenLock.Lock()
		if !p.distKeyGenImpl[keySetTypeEd25519].Finished() {
			p.distKeyGenLock.Unlock()
			return nil, errors.New("DistKeyGeneration procedure is not finished")
		}
		if !p.distKeyGenImpl[keySetTypeBLS].Finished() {
			p.distKeyGenLock.Unlock()
			return nil, errors.New("DistKeyGeneration procedure is not finished")
		}
		var distKeyShareDSS *rabin_dkg.DistKeyShare
		var distKeyShareBLS *rabin_dkg.DistKeyShare
		if distKeyShareDSS, err = p.distKeyGenImpl[keySetTypeEd25519].DistKeyShare(); err != nil {
			p.distKeyGenLock.Unlock()
			return nil, err
		}
		if distKeyShareBLS, err = p.distKeyGenImpl[keySetTypeBLS].DistKeyShare(); err != nil {
			p.distKeyGenLock.Unlock()
			return nil, err
		}
		p.distKeyGenLock.Unlock()
	...
	return makePeerMessage(p.distKeyGenID, peering.ReceiverDkg, step, pubShareMsg), nil
}
...
func (p *proc) rabinStep7CommitAndTerminateMakeResp(step byte, initRecv *peering.PeerMessageGroupIn, recvMsgs multiKeySetMsgs) (*peering.PeerMessageData, error) {
	...
	if err := p.node.distKeyPartRegistryProvider.SaveDistKeyPart(p.distKeyPart); err != nil {
		return nil, err
	}
	return makePeerMessage(p.distKeyGenID, peering.ReceiverDkg, step, &initiatorStatusMsg{error: nil}), nil
}
...
func (p *proc) nodeInQUAL(kst keySetType, nodeIdx uint16) bool {
	if nodeIdx == 0 && p.distKeyGenImpl == nil {
		return true // If N=1, Idx=0 is in QUAL.
	}
	p.distKeyGenLock.RLock()
	for _, q := range p.distKeyGenImpl[kst].QUAL() {
...
func (p *proc) makeInitiatorPubShareMsg(step byte) (*initiatorPubShareMsg, error) {
	...
	// if dssPublicShareBytes, err = p.distKeyPart.DSSPublicShares()[*p.distKeyPart.GetIndex()].MarshalBinary(); err != nil {
	// 	return nil, err
	// }
	...
	// var dssSignature *dss.PartialSig // TODO: we have to add another DistKeyGeneration here to produce a nonce.
	// if dssSignature, err = p.distKeyPart.DSSSignShare(dssPublicShareBytes); err != nil {
	// 	return nil, err
	// }
			...
			if isDkgInitProcRecvMsg(recv.MsgType) {
				s.onceSent.Do(func() {
					...
					if edSentMsgs, err = s.makeSent(s.step, keySetTypeEd25519, s.initRecv, s.prevMsgs.GetEdMsgs()); err != nil {
						...
						s.markDone(makePeerMessage(s.proc.distKeyGenID, peering.ReceiverDkg, s.step, &initiatorStatusMsg{error: err}))
					}

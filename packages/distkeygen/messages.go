...
//
// This file contains message types, exchanged between the DistKeyGeneration nodes
...
const (
	...
	initiatorInitMsgType = peering.FirstUserMsgCode + 184 // Initiator -> Peer: init new DistKeyGeneration, reply with initiatorStatusMsgType.
	//
...
// isDkgRabinRoundMsg detects, if the received MsgType is RabinDistKeyGeneration Peer <-> Peer message type and splits it into components.
...
// makeDkgRabinMsgType creates a peeringMsgType out of components composing it for the Rabin DistKeyGeneration Peer <-> Peer messages.
...
// initiate the DistKeyGeneration process.
type initiatorInitMsg struct {
	step          byte                   `bcs:"export"`
	distKeyGenRef string                 `bcs:"export"` // Some unique string to identify duplicate initialization.
...
func (msg *initiatorInitMsg) Read(r io.Reader) error {
	...
	msg.distKeyGenRef = rr.ReadString()
...
func (msg *initiatorInitMsg) Write(w io.Writer) error {
	...
	ww.WriteString(msg.distKeyGenRef)
...
// initiatorStepMsg
//
// This is a message used to synchronize the DistKeyGeneration procedure by
// ensuring the lock-step, as required by the DistKeyGeneration algorithm
...
// by the DistKeyGeneration peers returning the shared public key.

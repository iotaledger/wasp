...
pub enum Protocol {
    ...
    Respondent = (NN_RESPONDENT) as isize,
}
...
impl Socket {
    ...
    pub fn new(protocol: Protocol) -> Result<Socket> {
        Socket::create_socket(AF_SP, protocol)
    }
    ...
    pub fn new_for_device(protocol: Protocol) -> Result<Socket> {
        Socket::create_socket(AF_SP_RAW, protocol)
    }

    fn create_socket(domain: c_int, protocol: Protocol) -> Result<Socket> {
        let socket = unsafe { nn_socket(domain, protocol.to_raw()) };
    ...
    pub fn bind(&mut self, addr: &str) -> Result<Endpoint> {
        match CString::new(addr.as_bytes()) {
            Err(_) => Err(Error::from_raw(EINVAL)),
            Ok(c_addr) => {
                let ret = unsafe { nn_bind(self.socket, c_addr.as_ptr()) };
    ...
    pub fn connect(&mut self, addr: &str) -> Result<Endpoint> {
        match CString::new(addr.as_bytes()) {
            Err(_) => Err(Error::from_raw(EINVAL)),
            Ok(c_addr) => {
                let ret = unsafe { nn_connect(self.socket, c_addr.as_ptr()) };
                error_guard!(ret);
                Ok(Endpoint::new(ret, self.socket))
            }
        }
    ...
    pub fn nb_read(&self, buf: &mut [u8]) -> Result<usize> {
        ...
        let ret = unsafe {
            nn_recv(self.socket, c_buf_ptr, c_buf_len, NN_DONTWAIT)
        };
    ...
    pub fn nb_read_to_end(&self, buf: &mut Vec<u8>) -> Result<usize> {
        ...
        unsafe { nn_freemsg(msg as *mut c_void) };
        Ok(ret)
    }
    ...
    pub fn nb_write(&self, buf: &[u8]) -> Result<usize> {
        ...
        let ret = unsafe {
            nn_send(self.socket, buf_ptr, buf_len, NN_DONTWAIT)
        };
    ...
    pub fn zc_write(&self, buf: &[u8]) -> Result<usize> {
        ...
        let len = buf.len();
        let ret = unsafe { nn_send(self.socket, ptr_addr, NN_MSG, 0) };
    ...
    pub fn allocate_msg<'a>(len: usize) -> Result<&'a mut [u8]> {
        unsafe {
            let ptr = nn_allocmsg(len as size_t, 0) as *mut u8;
    ...
    pub fn poll(request: &mut PollRequest, timeout: isize) -> Result<usize> {
        ...
        let ret = unsafe { nn_poll(nn_fds, len, timeout as c_int) };
    ...
    pub fn device(socket1: &Socket, socket2: &Socket) -> Result<()> {
        let ret = unsafe { nn_device(socket1.socket, socket2.socket) };
    ...
    fn set_socket_options_str(&self, level: c_int, option: c_int, val: &str) -> Result<()> {
        match CString::new(val.as_bytes()) {
            ...
            Ok(c_val) => {
                ...
                let ret = unsafe {
                    nn_setsockopt(self.socket, level, option, ptr, val.len() as size_t)
                };
    ...
    fn set_socket_options_buf(&self, level: c_int, option: c_int, val: &[u8]) -> Result<()> {
        ...
        let ret =
            unsafe { nn_setsockopt(self.socket, level, option, buf_ptr, buf_len) };
    ...
    fn get_socket_option_c_int(&self, level: c_int, option: c_int) -> Result<c_int> {
        ...
        let ret =
            unsafe { nn_getsockopt(self.socket, level, option, val_ptr, sz_ptr) };
        error_guard!(ret);
        Ok(val)
    }
    ...
    fn get_socket_option_str(&self, level: c_int, option: c_int, len: size_t) -> Result<CString> {
        ...
        CString::new(val).map_err(|_| Error::from_raw(EINVAL))
    }
    ...
    pub fn get_receive_fd(&mut self) -> Result<RawFd> {
        self.get_socket_option_c_int(NN_SOL_SOCKET, NN_RCVFD)
    ...
    pub fn get_send_fd(&mut self) -> Result<RawFd> {
        self.get_socket_option_c_int(NN_SOL_SOCKET, NN_SNDFD)
            .map(|v: c_int| v as RawFd)
    }
    ...
    pub fn get_socket_name(&mut self, len: usize) -> Result<String> {
        self.get_socket_option_str(NN_SOL_SOCKET, NN_SOCKET_NAME, len)
    ...
    pub fn subscribe(&mut self, topic: &[u8]) -> Result<()> {
        self.set_socket_options_buf(NN_SUB, NN_SUB_SUBSCRIBE, topic)
    }

    /// Defined on full `Sub` socket. Unsubscribes from a particular topic.
    pub fn unsubscribe(&mut self, topic: &[u8]) -> Result<()> {
        self.set_socket_options_buf(NN_SUB, NN_SUB_UNSUBSCRIBE, topic)
    }
...
impl io::Read for Socket {
    ...
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        let ret = unsafe { nn_recv(self.socket, c_buf_ptr, c_buf_len, 0) };
...
impl io::Write for Socket {
    ...
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        let buf_len = buf.len() as size_t;
        let buf_ptr = buf.as_ptr() as *const c_void;
        let ret = unsafe { nn_send(self.socket, buf_ptr, buf_len, 0) };
...
mod tests {
    ...
    fn protocol_matches_raw() {
        assert_eq!(NN_REQ, Req.to_raw());
        assert_eq!(NN_REP, Rep.to_raw());
        assert_eq!(NN_PUSH, Push.to_raw());
        assert_eq!(NN_PULL, Pull.to_raw());
        assert_eq!(NN_PAIR, Pair.to_raw());
        assert_eq!(NN_BUS, Bus.to_raw());
        assert_eq!(NN_PUB, Pub.to_raw());
        assert_eq!(NN_SUB, Sub.to_raw());
        assert_eq!(NN_SURVEYOR, Surveyor.to_raw());
        assert_eq!(NN_RESPONDENT, Respondent.to_raw());
    }

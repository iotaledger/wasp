...
// The builder automatically maintains `internal` outputs which holds on-chain total balances of native tokens: one AsUTXO
// for one non-zero balance of iotago.NativeTokenID.
// Whenever native tokens are moved to/form the chain, those internal AsUTXO are updated by consuming input/producing output.

// Package vmtxbuilder implements AnchorTransactionBuilder, an transaction builder used by the VM to construct
...
// Whenever native tokes are moved to/form the chain, those internal UTXO are updated by consuming input/producing output.
...
// When txbuilder is unable to maintain consisten state, it panics. The following panic code are possible:
...
// - ErrNotEnoughFundsForInternalDustDeposit when total number of iotas available is not enough for dust deposit
//   of the new internal output
package vmtxbuilder

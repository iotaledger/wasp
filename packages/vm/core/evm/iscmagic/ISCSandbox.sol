...
interface ISCSandbox {
    ...
    function getNativeTokenID(uint32 foundrySN) external view returns (NativeTokenID memory);

    // Get the token scheme of a L2-controlled native token, given its foundry serial number
    function getNativeTokenScheme(uint32 foundrySN) external view returns (NativeTokenScheme memory);

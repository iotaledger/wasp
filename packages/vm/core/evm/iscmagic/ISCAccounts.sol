...
interface ISCAccounts {
    ...
    function getL2BalanceBaseTokens(ISCAgentID memory agentID)
        external
        view
        returns (uint64);

    // Get the L2 native tokens balance of an account
    function getL2BalanceNativeTokens(
        NativeTokenID memory id,
        ISCAgentID memory agentID
    ) external view returns (uint256);

    // Get the L2 NFTs of an account
    function getL2NFTs(ISCAgentID memory agentID)
        external
        view
        returns (NFTID[] memory);
}

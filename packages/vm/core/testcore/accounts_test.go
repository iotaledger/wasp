...
func TestAccounts_Deposit(t *testing.T) {
	...
	sender, _ := env.NewKeyPairWithFunds(env.NewSeedFromTestName(t.Name()))
	ch := env.NewChain()
...
func TestAccounts_DepositCheatAllowance(t *testing.T) {
	env := solo.New(t, &solo.InitOptions{})
	sender, senderAddr := env.NewKeyPairWithFunds(env.NewSeedFromTestName(t.Name()))
...
// 		senderKeyPair, senderAddr = env.NewKeyPairWithFunds(env.NewSeedFromTestName(t.Name()))
// 		senderAgentID = isc.NewAddressAgentID(senderAddr)
...
func initDepositTest(t *testing.T, initCommonAccountBaseTokens ...coin.Value) *accountsDepositTest {
	...
	ret.chainOwner, ret.chainOwnerAddr = ret.env.NewKeyPairWithFunds(ret.env.NewSeedFromTestName(t.Name()))
	ret.chainOwnerAgentID = isc.NewAddressAgentID(ret.chainOwnerAddr)
	ret.user, ret.userAddr = ret.env.NewKeyPairWithFunds(ret.env.NewSeedFromTestName(t.Name()))
...
func TestAccounts_TransferPartialAssets(t *testing.T) {
	...
	// send funds to user2
	user2, user2Addr := v.env.NewKeyPairWithFunds(v.env.NewSeedFromTestName(t.Name()))
...
func TestAccounts_DepositNFTWithMinStorageDeposit(t *testing.T) {
	...
	// ch := env.NewChain()
	//
	// issuerWallet, issuerAddress := env.NewKeyPairWithFunds(env.NewSeedFromTestName(t.Name())	//
...
func TestAccounts_DepositWithNoGasBudget(t *testing.T) {
	env := solo.New(t, &solo.InitOptions{})
	senderWallet, _ := env.NewKeyPairWithFunds(env.NewSeedFromTestName(t.Name()))
	ch := env.NewChain()
...
// 	wallet, _ := env.NewKeyPairWithFunds(env.NewSeedFromTestName(t.Name()))))
// 	_, anotherUserAddr := env.NewKeyPairWithFunds(env.NewSeedFromTestName(t.Name()))))
...
// 	wallet, address := env.NewKeyPairWithFunds(env.NewSeedFromTestName(t.Name()))))

...
var exportMap = wasmlib.ScExportMap{
	...
	Funcs: []wasmlib.ScFuncContextFunction{
		...
		funcDoNothingThunk,
		funcEstimateMinStorageDepositThunk,
		funcIncCounterThunk,
...
type EstimateMinStorageDepositContext struct {
	State MutableTestCoreState
}

func funcEstimateMinStorageDepositThunk(ctx wasmlib.ScFuncContext) {
	ctx.Log("testcore.funcEstimateMinStorageDeposit")
	f := &EstimateMinStorageDepositContext{
		State: MutableTestCoreState{
			proxy: wasmlib.NewStateProxy(),
		},
	}
	funcEstimateMinStorageDeposit(ctx, f)
...
func funcWithdrawFromChainThunk(ctx wasmlib.ScFuncContext) {
	...
	ctx.Require(f.Params.BaseTokensWithdrawal().Exists(), "missing mandatory baseTokensWithdrawal")
	ctx.Require(f.Params.ChainID().Exists(), "missing mandatory chainID")

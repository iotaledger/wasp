...
func passTypesFull(ctx isc.Sandbox) isc.CallArguments {
	...
	s_0, err := isc.ArgAt[string](params, 1)
	checkFull(ctx, err)
	if s_0 != "" {
		ctx.Log().Panicf("wrong string_0")
	}

	i64, err := isc.ArgAt[int64](params, 2)
	checkFull(ctx, err)
	if i64 != 42 {
		ctx.Log().Panicf("wrong int64")
	}

	i64_0, err := isc.ArgAt[int64](params, 3)
	checkFull(ctx, err)
	if i64_0 != 0 {
		ctx.Log().Panicf("wrong int64_0")
	}

	hash, err := isc.ArgAt[hashing.HashValue](params, 4)
	checkFull(ctx, err)
	if hash != hashing.HashStrings("Hash") {
		ctx.Log().Panicf("wrong hash")
	}
	hname, err := isc.ArgAt[isc.Hname](params, 5)
	checkFull(ctx, err)
	if hname != isc.Hn("Hname") {
		ctx.Log().Panicf("wrong hname")
	}
	hname0, err := isc.ArgAt[isc.Hname](params, 6)
	checkFull(ctx, err)
	if hname0 != 0 {
		ctx.Log().Panicf("wrong Hname-0")
	}
	_, err = isc.ArgAt[isc.AgentID](params, 7)
	checkFull(ctx, err)

	_, err = isc.ArgAt[isc.ChainID](params, 8)
	checkFull(ctx, err)

	_, err = isc.ArgAt[*cryptolib.Address](params, 9)
	checkFull(ctx, err)

	_, err = isc.ArgAt[isc.AgentID](params, 10)
	checkFull(ctx, err)
	return nil
}

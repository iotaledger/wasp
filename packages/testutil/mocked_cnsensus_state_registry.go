...
func (s *mockedConsensusStateRegistry) Get(chainID isc.ChainID, committeeAddr *cryptolib.Address) (*committeelog.State, error) {
	if store, ok := s.data[committeeAddr.Key()]; ok {
		return store, nil
	}
	return nil, committeelog.ErrCommitteeLogStateNotFound
}

func (s *mockedConsensusStateRegistry) Set(chainID isc.ChainID, committeeAddr *cryptolib.Address, state *committeelog.State) error {
	s.data[committeeAddr.Key()] = state
	return nil
}

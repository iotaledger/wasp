...
func (env *Solo) SuiClient() *suiclient2.Client {
	return suiclient2.NewHTTP(env.l1Config.SuiRPCURL)
}
...
func (env *Solo) GetFundsFromFaucet(target *cryptolib.Address) {
	err := suiclient2.RequestFundsFromFaucet(env.ctx, target.AsSuiAddress(), env.l1Config.SuiFaucetURL)
	require.NoError(env.T, err)
	env.AssertL1BaseTokens(target, suiclient2.FundsFromFaucetAmount)
}

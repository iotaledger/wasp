...
package batch_proposal_agreement_test
...
import (
	...
	"github.com/iotaledger/wasp/v2/packages/gpa/batch_proposal_agreement"
...
func testBasic(t *testing.T, n, f, silent int) {
	...
	for i, nid := range nodeIDs {
		if i >= n-silent {
			...
			nodes[nid] = batch_proposal_agreement.New(nodeIDs, nid, f, makeCCInstFun, nodeLog).AsGPA()
		}
	}
	...
	//
	out0 := nodes[nodeIDs[0]].Output().(*batch_proposal_agreement.Output)
	for i, nid := range nodeIDs {
		if i >= n-silent {
			continue
		}
		out := nodes[nid].Output().(*batch_proposal_agreement.Output)

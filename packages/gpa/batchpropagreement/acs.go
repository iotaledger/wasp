...
import (
	...
	"github.com/iotaledger/wasp/v2/packages/gpa/rbc/broadcast"
)

type BatchProposalAgreement interface {
	AsGPA() gpa.GPA
}
...
const (
	subsystemReliableBroadcast byte = iota
	subsystemABA
)
...
type acsImpl struct {
	...
	reliableBroadcastInsts   map[gpa.NodeID]gpa.GPA // RBC instances.
	reliableBroadcastInput   bool                   // Have we provided our input?
	reliableBroadcastOutputs map[gpa.NodeID][]byte  // Outputs received from the RBC.
...
var (
	_ gpa.GPA                = &acsImpl{}
	_ BatchProposalAgreement = &acsImpl{}
)
...
// > where P_i is the sender of RBC_i. Let {BA_i}_N refer to N instances
// > of the binary byzantine agreement protocol.
func New(nodeIDs []gpa.NodeID, me gpa.NodeID, f int, ccCreateFun func(node gpa.NodeID, round int) gpa.GPA, log log.Logger) BatchProposalAgreement {
	...
	for i, nid := range nodeIDs {
		...
		nodeIdx[nid] = i
		rbcInsts[nid] = broadcast.NewReliableBroadcast(nodeIDs, f, me, nid, math.MaxInt, func(b []byte) bool { return true }, log) // TODO: MaxInt.
	...
	a := &acsImpl{
		...
		n:                        n,
		f:                        f,
		reliableBroadcastInsts:   rbcInsts,
		reliableBroadcastInput:   false,
		reliableBroadcastOutputs: map[gpa.NodeID][]byte{},
		abaInsts:                 abaInsts,
...
func (a *acsImpl) selectSubsystem(subsystem byte, index int) (gpa.GPA, error) {
	...
	nid := a.nodeIDs[index]
	switch subsystem {
	case subsystemReliableBroadcast:
		return a.reliableBroadcastInsts[nid], nil
	case subsystemABA:
...
func (a *acsImpl) Input(input gpa.Input) gpa.OutMessages {
	...
	if a.reliableBroadcastInput {
		return nil // Duplicate input.
	}
	a.reliableBroadcastInput = true
	msgs := gpa.NoMessages()
	sub, subMsgs, err := a.msgWrapper.DelegateInput(subsystemReliableBroadcast, a.nodeIdx[a.me], input)
	if err != nil {
...
func (a *acsImpl) tryHandleRBCOutput(nodeID gpa.NodeID, rbcInst gpa.GPA) gpa.OutMessages {
	...
	if _, ok := a.reliableBroadcastOutputs[nodeID]; ok {
		return nil // Already handled.
	}
	a.reliableBroadcastOutputs[nodeID] = out.([]byte)
	a.tryOutput()
...
func (a *acsImpl) tryOutput() {
	...
	for nid, abaOut := range a.abaOutputs {
		if abaOut {
			if rbcOut, ok := a.reliableBroadcastOutputs[nid]; ok {
				values[nid] = rbcOut
				continue
			}
...
func (a *acsImpl) StatusString() string {
	if a.output != nil {
		return fmt.Sprintf(
			...
			len(a.output.Values), a.output.Terminated, a.n, a.f, len(a.reliableBroadcastOutputs), len(a.abaOutputs),
		)
	}
	return fmt.Sprintf(
		"{ACS, out=nil, n=%v, f=%v, |rbcOut|=%v, |abaOut|=%v}",
		a.n, a.f, len(a.reliableBroadcastOutputs), len(a.abaOutputs),
	)
}

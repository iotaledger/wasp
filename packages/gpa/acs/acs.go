...
type ACS struct {
	...
	rbcInsts   map[gpa.NodeID]*gpa.OwnHandler[*bracha.RBC]     // RBC instances.
	rbcInput   bool                                            // Have we provided our input?
	rbcOutputs map[gpa.NodeID][]byte                           // Outputs received from the RBC.
	abaInsts   map[gpa.NodeID]*gpa.OwnHandler[*mostefaoui.ABA] // ABA Instances.
...
func New(nodeIDs []gpa.NodeID, me gpa.NodeID, f int, ccCreateFun func(node gpa.NodeID, round int) gpa.GPA, log log.Logger) *ACS {
	...
	rbcInsts := map[gpa.NodeID]*gpa.OwnHandler[*bracha.RBC]{}
	abaInsts := map[gpa.NodeID]*gpa.OwnHandler[*mostefaoui.ABA]{}

...
type TestContextNewFunctors[Obj any, Input any, Msg MessageNew] struct {
...
type TestContextNew[Obj any, Input any, Msg MessageNew] struct {
	...
	msgCh           <-chan SenderMessageNew[Msg]       // A way to provide additional messages w/o synchronizing other parts.
	msgs            []SenderMessageNew[Msg]            // Not yet delivered messages.
	msgsSent        int                                // Stats.
	msgsRecv        int                                // Stats.
}

func NewTestContextNew[Obj any, Input any, Msg MessageNew](
...
func (tc *TestContextNew[Obj, Input, Message]) WithMessageChannel(msgCh <-chan SenderMessageNew[Message]) *TestContextNew[Obj, Input, Message] {
	tc.msgCh = msgCh
	return tc
}
...
func (tc *TestContextNew[Obj, Input, Message]) setMessageSender(sender NodeID, msgs []Message) []SenderMessageNew[Message] {
	if msgs == nil {
		return nil
	}
	result := make([]SenderMessageNew[Message], len(msgs))
	for i := range msgs {
		msgs[i].SetSender(sender)
		result[i] = SenderMessageNew[Message]{Sender: sender, Message: lo.ToPtr(msgs[i])}
	}
	return result
}
...
type SenderMessageNew[Msg MessageNew] struct {
	Sender  NodeID
	Message *Msg
}

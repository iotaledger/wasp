...
func TestBasic(t *testing.T) {
	test := func(tt *testing.T, n, f int) {
		...
		for _, nid := range nodeIDs {
			nodes[nid] = broadcast.NewReliableBroadcast(nodeIDs, f, nid, leader, math.MaxInt, func(b []byte) bool { return true }, gpa.NewPanicLogger())
		}
...
func TestPredicate(t *testing.T) {
	...
	test := func(tt *testing.T, n, f int) {
		...
		nodes := map[gpa.NodeID]gpa.GPA{}
		for _, nid := range nodeIDs {
			nodes[nid] = broadcast.NewReliableBroadcast(nodeIDs, f, nid, leader, math.MaxInt, pFalse, gpa.NewPanicLogger()) // NOTE: Initially false.
		}
		//

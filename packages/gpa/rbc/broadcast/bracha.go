...
var _ gpa.GPA = &reliableBroadcast{}

// NewReliableBroadcast creates new instance of the RBC.
func NewReliableBroadcast(peers []gpa.NodeID, f int, me, broadcaster gpa.NodeID, maxMsgSize int, predicate func([]byte) bool, log gpa.Logger) gpa.GPA {
	r := &reliableBroadcast{
		n:           len(peers),
		f:           f,
		me:          me,
...
//	02: input 𝑀
//	03: send ⟨PROPOSE, 𝑀⟩ to all
func (r *reliableBroadcast) Input(input gpa.Input) gpa.OutMessages {
...
// Implements the GPA interface.
func (r *reliableBroadcast) Message(msg gpa.Message) gpa.OutMessages {
...
//	07:     if 𝑃(𝑀) then
//	08:         send ⟨ECHO, 𝑀⟩ to all
func (r *reliableBroadcast) handlePropose(msg *msgBracha) gpa.OutMessages {
...
//	09: upon receiving 2𝑡 + 1 ⟨ECHO, 𝑀⟩ messages and not having sent a READY message do
//	10:     send ⟨READY, 𝑀⟩ to all
func (r *reliableBroadcast) handleEcho(msg *msgBracha) gpa.OutMessages {
	//
...
//	13: upon receiving 2𝑡 + 1 ⟨READY, 𝑀⟩ messages do
//	14:     output 𝑀
func (r *reliableBroadcast) handleReady(msg *msgBracha) gpa.OutMessages {
	//
	// Mark the message as received.
	h := r.valueHash(msg)
...
func (r *reliableBroadcast) checkMsgRecv(msg *msgBracha) bool {
...
func (r *reliableBroadcast) markEchoRecv(h hashing.HashValue, msg *msgBracha) {
...
func (r *reliableBroadcast) markReadyRecv(h hashing.HashValue, msg *msgBracha) {
...
func (r *reliableBroadcast) maybeSendReady(v []byte) gpa.OutMessages {
	if r.readySent {
		return nil
	}
...
func (r *reliableBroadcast) sendToAll(brachaType msgBrachaType, value []byte) gpa.OutMessages {
...
func (r *reliableBroadcast) valueHash(msg *msgBracha) hashing.HashValue {
	return hashing.HashData(msg.value)
}

// Implements the GPA interface.
func (r *reliableBroadcast) Output() gpa.Output {
	if r.output == nil {
...
// Implements the GPA interface.
func (r *reliableBroadcast) StatusString() string {
	return fmt.Sprintf(
...
// Implements the GPA interface.
func (r *reliableBroadcast) UnmarshalMessage(data []byte) (gpa.Message, error) {

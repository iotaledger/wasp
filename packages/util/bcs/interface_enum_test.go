...
func TestBasicInterfaceEnumCodec(t *testing.T) {
	...
	testCodecNoRef(t, StructWithEnum(InfEnum1(vS)), refEnumEnc)

	vI := int32(42)
	refEnumEnc = ref_bcs.MustMarshal(RefEnum2{B: &vI})
	require.NotEmpty(t, refEnumEnc)
	testCodecNoRef(t, StructWithEnum(InfEnum2(vI)), refEnumEnc)
...
func TestInterfaceEnumVariantWithCustomCodec(t *testing.T) {
	...
	testCodecNoRef(t, lo.ToPtr[InfEnum1](WithCustomCodec{}), []byte{0x0, 0x1, 0x2, 0x3})
	testCodecNoRef(t, lo.ToPtr[InfEnum1]("aaa"), []byte{0x1, 0x3, 0x61, 0x61, 0x61})
}
...
func TestIntEnumWithStructEnumVariant(t *testing.T) {
	...
	testCodecNoRef(t, lo.ToPtr[InfEnum1](BasicStructEnum{A: lo.ToPtr[int32](10)}), []byte{0x0, 0x0, 0xa, 0x0, 0x0, 0x0})
	testCodecNoRef(t, lo.ToPtr[InfEnum1](BasicStructEnum{B: lo.ToPtr("aaa")}), []byte{0x0, 0x1, 0x3, 0x61, 0x61, 0x61})
	testCodecNoRef(t, lo.ToPtr[InfEnum1](WithCustomCodec{}), []byte{0x1, 0x1, 0x2, 0x3})
}

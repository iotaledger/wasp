...
func provide(c *dig.Container) error {
	...
	if err := c.Provide(func(deps consensusRegistryDeps) committeelog.ConsensusStateRegistry {
	...
	type distKeyPartsRegistryDeps struct {
		dig.In
	...
	if err := c.Provide(func(deps distKeyPartsRegistryDeps) registry.DistKeyPartRegistryProvider {
		distKeyPartsRegistry, err := registry.NewDistKeyPartsRegistry(ParamsRegistries.DistKeyParts.Path, deps.NodeIdentityProvider.NodeIdentity().GetPrivateKey())
		if err != nil {
			Component.LogPanic(err.Error())
		}
		return distKeyPartsRegistry
	}); err != nil {
		Component.LogPanic(err.Error())
	}

...
func TestGetChainIdentifier(t *testing.T) {
	...
	require.Equal(t, suiconn.ChainIdentifierAlphanet, chainID)
}
...
func TestGetCheckpoint(t *testing.T) {
	...
	targetCheckpoint := &suijsonrpc.Checkpoint{
		...
		Digest:                   *sui.MustNewDigest("Eu7yhUZ1oma3fk8KhHW86usFvSmjZ7QPEhPsX7ZYfRg3"),
		NetworkTotalTransactions: suijsonrpc.NewBigInt(1004),
		PreviousDigest:           sui.MustNewDigest("AcrgtLsNQxZQRU1JK395vanZzSR6nTun6huJAxEJuk14"),
		...
		TimestampMs:           suijsonrpc.NewBigInt(1725548499477),
		Transactions:          []*sui.Digest{sui.MustNewDigest("8iu72fMHEFHiJMfjrPDTKBPufQgMSRKfeh2idG5CoHvE")},
		CheckpointCommitments: []sui.CheckpointCommitment{},
		ValidatorSignature:    *sui.MustNewBase64Data("k0u7tZR87vS8glhPgmCzgKFm1UU1ikmPmO9nVzFXn9XY20kpftc6zxdBe0lmSAzs"),
	}
...
func TestGetCheckpoints(t *testing.T) {
	...
	checkpointPage, err := client.GetCheckpoints(
		context.Background(), suiclient.GetCheckpointsRequest{
			Cursor: cursor,
			Limit:  &limit,
		},
	)
...
func TestTryGetPastObject(t *testing.T) {
	...
	resp, err := api.TryGetPastObject(
		context.Background(), suiclient.TryGetPastObjectRequest{
...
func TestTryMultiGetPastObjects(t *testing.T) {
	...
	resp, err := api.TryMultiGetPastObjects(
		context.Background(), suiclient.TryMultiGetPastObjectsRequest{
			PastObjects: req,
			Options: &suijsonrpc.SuiObjectDataOptions{
				ShowType:  true,
				ShowOwner: true,

...
func TestGetOwnedObjects(t *testing.T) {
	...
	fundedSigner := iotatest.MakeSignerWithFunds(0, iotaconn.AlphanetFaucetURL)
	unfundedSigner := iotasigner.NewSignerByIndex(testcommon.TestSeed, iotasigner.KeySchemeFlagDefault, 0)
	...
	t.Run(
		"struct tag", func(t *testing.T) {
			...
			require.NoError(t, err)
			require.GreaterOrEqual(t, len(objs.Data), 1, "Expected at least 1 IOTA coin object")
		},
	)
	...
	t.Run(
		"move module", func(t *testing.T) {
			query := iotajsonrpc.IotaObjectResponseQuery{
				Filter: &iotajsonrpc.IotaObjectDataFilter{
					AddressOwner: unfundedSigner.Address(),
				},
				Options: &iotajsonrpc.IotaObjectDataOptions{
					ShowType:    true,
					ShowContent: true,
			...
			objs, err := client.GetOwnedObjects(
				context.Background(), iotaclient.GetOwnedObjectsRequest{
					Address: unfundedSigner.Address(),
					Query:   &query,
					Limit:   &limit,
				},
			)

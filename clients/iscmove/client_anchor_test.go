...
func TestReceiveAndUpdateStateRootRequest(t *testing.T) {
	...
	require.NoError(t, err)
	sentAssetsBagRef, err := txnResponse.GetCreatedObjectInfo(types.AssetsBagModuleName, types.AssetsBagObjectName)
	require.NoError(t, err)
	...
	requestRef, err := createAndSendRequestRes.GetCreatedObjectInfo(types.RequestModuleName, types.RequestObjectName)
	require.NoError(t, err)
...
func startNewChain(t *testing.T, client *iscmove.Client, signer cryptolib.Signer, iscPackageID sui.PackageID) *types.RefWithObject[types.Anchor] {
	anchor, err := client.StartNewChain(
		context.Background(),
		signer,
		iscPackageID,

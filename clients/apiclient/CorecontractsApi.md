...
> IotaObject accountsGetObjectData()
...
```typescript
import { createConfiguration, CorecontractsApi } from '';
import type { CorecontractsApiAccountsGetObjectDataRequest } from '';

const configuration = createConfiguration();
const apiInstance = new CorecontractsApi(configuration);

const request: CorecontractsApiAccountsGetObjectDataRequest = {
    // Object ID (Hex)
...
const data = await apiInstance.accountsGetObjectData(request);
console.log('API called successfully. Returned data:', data);
```
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **objectID** | [**string**] | Object ID (Hex) | defaults to undefined
 **block** | [**string**] | Block index or trie root | (optional) defaults to undefined

...
export class CorecontractsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param block Block index or trie root
     */
    public async accountsGetObjectData(objectID: string, block?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'objectID' is not null or undefined
        if (objectID === null || objectID === undefined) {
            throw new RequiredError("CorecontractsApi", "accountsGetObjectData", "objectID");
        }



        // Path Params
        const localVarPath = '/v1/chain/core/accounts/objectdata/{objectID}'
...
export class CorecontractsApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to accountsGetObjectData
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async accountsGetObjectDataWithHttpInfo(response: ResponseContext): Promise<HttpInfo<IotaObject >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: IotaObject = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "IotaObject", ""
            ) as IotaObject;
            return new HttpInfo(response.httpStatusCode, response.headers, response.body, body);
